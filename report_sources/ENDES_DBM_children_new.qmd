...
source(here::here("scripts", "dictionary_factors.R"))
source(here::here("scripts", "statistics and raincloud plots.R"))
...
```{r}
# Child's data
data_2015_2024 <- rio::import(here("data", "clean", "data_endes_2015_2024.dta"))
...
labelled::var_label(data_2015_2024)  # List of variable labels (if available)
Hmisc::contents(data_2015_2024)      # data.frame with variable types and missing values
dplyr::glimpse(data_2015_2024)       # Compact structure of variable names and types
str(data_2015_2024)                  # Detailed structure of the data.frame
```

```{r}
# Select and clean variables
data_dbm <- data_2015_2024 |>
  dplyr::select(
    edad_meses_minsa_cat, sexo_infante, sexo_jefe, region_a, region, residencia, 
    quintil, educacion_madre, altura, dcronica, wasting, thinness, anemia, dbm, 
    agua_tipo, agua_tratamiento, agua_disponible, HV024, weights, cluster, stratum, 
    talla_edad_oms, peso_edad_oms, peso_talla_oms, HV024, talla_edad_oms, 
    peso_edad_oms, peso_talla_oms, year, weight_scaled)
```

## Dictionary: Recode, relevel, and relabel variables

```{r}
data_dbm <- my_dictionary_factors(data_dbm)
```
...
```{r}
#| eval: false
# Complex survey design by stratified cluster with svydesign
...
prop.table(
  table(
    design_test$variables$dbm,
    design_test$variables$residencia
  ),
  margin = 1
)
...
purrr::map(c(
  "edad_meses_minsa_cat", "sexo_infante", "sexo_jefe", "region_a", "region", 
  "residencia", "quintil", "educacion_madre", "altura", "agua_tipo", 
  "agua_tratamiento", "agua_disponible", "dcronica", "anemia", "wasting", 
  "thinness", "year", "departamento"), ~ categorical_vars |>
  tabyl(.data[[.x]], dbm) |>
  adorn_totals(where = "both") |>
  adorn_percentages(denominator = "col") |>
  adorn_pct_formatting() |>
...
numerical_vars <- data_dbm |>
  dplyr::select(where(\(x) is.numeric(x)), dbm)
  # mutate(across(
  #   c(talla_edad_oms, peso_edad_oms, peso_talla_oms),
  #   ~ replace(.x, .x > 9990, NA)
  # ))
```
...
::: {style="text-align: justify"}
El test de Shapiro-Wilk evalúa la normalidad de un conjunto de datos, pero en muestras grandes su cálculo se vuelve inestable, detectando desviaciones mínimas como significativas y tiende a rechazar la normalidad incluso cuando la distribución es aproximadamente normal. En la práctica, la función `shapiro.test()` en R tiene un límite de 5,000 observaciones, ya que por encima de este umbral pierde utilidad diagnóstica. Por otro lado, el test de Anderson-Darling (`ad.test`) es más sensible a la forma de las colas de la distribución, aunque su cálculo es computacionalmente más costoso, pues requiere ordenar los datos y aplicar una suma ponderada de diferencias. En contextos con muestras masivas, la normalidad suele asumirse por el Teorema del Límite Central, salvo que existan evidencias de fuerte asimetría o colas pesadas. Como alternativa, se recomienda complementar con histogramas, raincloud plots, QQ-plots y métricas como skewness y kurtosis.
::: 

```{r fig.height=9, fig.width=9, fig.align = 'center'}
group_stat_table_plot(numerical_vars, "talla_edad_oms", "dbm")
```
...
```{r}
# Complex survey design by stratified clusters
data_dbm_svy <- srvyr::as_survey_design(
  data_dbm, # have to specify the dataset
  ids = cluster, # cluster ids (PSU) + household ids (SSU)
  strata = stratum, # sampling was stratified by district
  weights = weight_scaled, # Scaled weights
  nest = TRUE
)
```
...
table_1a <- data_dbm_svy |>
  tbl_svysummary(
    include = c(
      edad_meses_minsa_cat, sexo_infante, region_a, residencia, quintil,
      educacion_madre, altura, agua_tipo, dcronica, anemia, dbm),
...
table_1 <- tbl_merge(
  ...
  modify_footnote(
    stat_0_1 ~ "Unweighted count",
    stat_0_2 ~ "Weighted percentage")
...
data_dbm |>
  dplyr::select(
    edad_meses_minsa_cat, sexo_infante, region_a, residencia, quintil,
    educacion_madre, altura, agua_tipo, dcronica, anemia, dbm, weights
  ) |>
  pivot_longer(
    cols = c(edad_meses_minsa_cat:dbm),
    names_to = "Variable",
    values_to = "Valor"
  ) |>
  dplyr::group_by(Variable, Valor) |>
  dplyr::filter(!is.na(Valor)) |>
  dplyr::summarise(n = n(), weighted_prop = sum(weights)) |>
...
## Figure 1: Prevalence with `tableGrob` and `ggplot2`

### Table with `srvyr` and `dplyr`
...
# Combine tables for Figure 1
tabla_area_total <- bind_rows(tabla_area, tabla_total) |>
  mutate(area = factor(area, levels = c("Total", "Rural", "Urbano")))

# Wide format for Table 1
...
grid.table(
  ...
  theme = ttheme_minimal(
    core = list(fg_params = list(fontface = "plain", hjust = 0.5)),
    colhead = list(fg_params = list(fontface = "bold", col = "gray20"))
  )
)
```

```{r}
# Generate grob table without header rows/columns
...
tabla_plot <- ggplot() +
  ...
  theme(
    plot.margin = margin(t = 0, b = 0, r = 15, l = 0),
...
F1_final <- grid.arrange(F1_adjusted, tabla_plot, ncol = 1, heights = c(3, 0.5))
```

### `patchwork` figure and table
...
#| fig-width: 10
# Table without area
tabla_merge_patch <- tabla_merge |>
  dplyr::select(-Area)

# Grob table without header
tabla_grob_patch <- tableGrob(tabla_merge_patch, rows = NULL, cols = NULL)
tabla_grob_patch$widths <- unit(rep(1, ncol(tabla_grob_patch)), "null")
tabla_grob_patch$heights <- unit(rep(1, nrow(tabla_grob_patch)), "null")

# Table as empty plot to combine
tabla_grob_patch <- ggplot() +
  annotation_custom(tabla_grob_patch) +
  theme_minimal() +
  theme(
    plot.margin = margin(t = -15, b = 0)
  )

# Figura principal debe tener margen inferior
F1_adjusted <- F1 +
  theme(
...
ggsave(
  F1, 
  filename = here("outputs", "prevalence_plot.png"), 
  width = 10, 
  height = 7, 
  dpi = 300)
...
```{r}
paleta_rosa <- c("#fde0dd", "#fbb4b9", "#f768a1", "#c51b8a", "#7a0177")
```

```{r}
# intervalos de confianza como áreas sombreadas
ggplot(tabla_dbm, aes(x = year, y = prevalencia, color = area)) +
  geom_ribbon(
    aes(ymin = lower, ymax = upper, fill = area),
    alpha = 0.2,
    colour = NA
  ) +
  geom_line(aes(group = area), linewidth = 1.5) +
  geom_point(aes(shape = area), size = 4) +
  geom_text_repel(
    aes(label = round(prevalencia, 1)),
    size = 3.5,
    box.padding = 0.7,
    family = "Syne"
  ) +
  scale_y_continuous(breaks = seq(0, 15, by = 3)) +
  labs(color = "Área de residencia", shape = "Área", fill = "Área de residencia") +
  ggsci::scale_color_bmj() +
  ggsci::scale_fill_bmj() +
  theme_minimal(base_size = 14, base_family = "Syne") +
  theme(
    axis.title = element_blank(),
    legend.position = "bottom",
    text = element_text(color = "black"),
    panel.grid.minor = element_blank(),
    panel.grid.major = element_line(color = "#d0d0d0", linetype = "dashed"),
