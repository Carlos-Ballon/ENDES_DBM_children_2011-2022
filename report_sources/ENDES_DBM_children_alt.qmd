...
```{r}
# Child's data
data_ninios <- rio::import(here("data", "ninios-2011-2022_tmp.dta"))
...
lapply(categorical_vars, function(x) sum(complete.cases(x)))

# Columns without missing values (vector)
colnames(categorical_vars)[sapply(categorical_vars, function(x) all(!is.na(x)))]
```
...
lapply(categorical_vars, function(x) unique(x))
...
lapply(categorical_vars, function(x) addmargins(table(x)))

# Relative frequency for categorical variables
lapply(categorical_vars, function(x) round(addmargins(prop.table(table(x)) * 100), 1))
...
prop.table(table(data_dbm$DBM_anemia_stunting_factor, data_dbm$area_residencia_factor), margin = 1)

prop.table(table(diseño$variables$DBM_anemia_stunting_factor, diseño$variables$area_residencia_factor), margin = 1)
...
numerical_vars <- data_dbm |>
  dplyr::select(
    edad_meses, talla_edad, talla_edad_oms, peso_edad, peso_edad_oms, peso_talla,
    peso_talla_oms, DBM_anemia_stunting_factor) 
```
...
numerical_vars |> 
  ...
  lapply(function(df) summ(df, digits = 2, na.rm = TRUE))
```
...
tabla_area <- data_dbm_svy |>
  ...
  dplyr::summarise(prevalencia = srvyr::survey_mean(
  ...
  mutate(
    ...
    prevalencia = round(prevalencia * 100, 1)) |>
...
tabla_area_total <- bind_rows(tabla_area, tabla_total) |>
  dplyr::arrange(year, factor(area, levels = c("Total", "Rural", "Urbano")))
...
# Crear tabla con encabezado desactivado pero estilo minimal
tabla_merge_theme <- ggtexttable(
  tabla_merge,
...
# Añadir líneas verticales de izquierda a derecha en toda la tabla
tabla_merge_theme <- tab_add_vline(
  tab = tabla_merge_theme,
  at.column = 2:tab_ncol(tabla_merge_theme),
  column.side = "left",
  from.row = 1,
  to.row = tab_nrow(tabla_merge_theme),
  linetype = 1, 
  linewidth = 1,
  linecolor = "black"
)
...
tabla_plot <- ggplot() +
  ...
  theme_minimal() +
  theme(
    plot.margin = margin(t = -15, b = 0)  # ajusta posición de tabla más cerca del gráfico
  )

# Figura principal (F1) debe tener margen inferior
F1_adjusted <- F1 +
  theme(
    plot.margin = margin(t = 10, b = 15, l = 10, r = 10),
...
#| fig-height: 7
#| fig-width: 10
etiquetas_area <- tabla_merge$Area
posiciones_y <- seq(length(etiquetas_area), 1)  # de arriba hacia abajo

anotaciones_plot <- ggplot() +
  geom_text(
    aes(x = 0, y = posiciones_y, label = etiquetas_area),
  ...
  theme_void() +
  theme(
    plot.margin = margin(t = -15, r = 0, l = 10),
    plot.background = element_rect(fill = "transparent", color = NA)
  )
...
tabla_grob <- tableGrob(tabla_merge_t, rows = NULL, cols = NULL)
tabla_grob$widths <- unit(rep(1, ncol(tabla_merge_t)), "null")
tabla_grob$heights <- unit(rep(1, nrow(tabla_merge_t)), "null")

tabla_plot <- ggplot() +
  annotation_custom(tabla_grob) +
  theme_void() +
  theme(plot.margin = margin(t = -15, b = 0))

tabla_combinada <- anotaciones_plot | tabla_plot +
  plot_layout(widths = c(1.2, 9))  # Ajusta proporción anotaciones : tabla

F1_adjusted / tabla_combinada +
  plot_layout(heights = c(4, 1.3)) &
  theme(
    plot.background = element_rect(fill = "transparent", color = NA),
    panel.background = element_rect(fill = "transparent", color = NA)
...
#| fig-height: 7
#| fig-width: 10
tabla_valores <- tabla_merge |>
  dplyr::select(-Area)

tabla_area <- tabla_merge |>
  dplyr::select(Area)

tabla_completa_df <- cbind(tabla_area, tabla_valores)

tabla_grob <- tableGrob(tabla_completa_df, rows = NULL, cols = NULL)
tabla_grob$widths <- unit(c(1, rep(1, ncol(tabla_completa_df) - 1)), "null")
tabla_grob$heights <- unit(rep(1, nrow(tabla_completa_df)), "null")
...
grid.arrange(F1_adjusted, tabla_plot, ncol = 1, heights = c(4, 1.3))
```
...
tabla_area_total <- data_dbm |>
  ...
  dplyr::summarise(n = n(), weighted_prop = sum(pesos), .groups = "drop_last") |>
  mutate(
    weighted_prop = round(weighted_prop / sum(weighted_prop) * 100, 2),
